...
class BatchProcessor(BaseProcessor):
    ...
    def __init__(self, db_session: Optional[Session] = None):
        super().__init__(db_session, "batch")
    ...
    def process(self, json_data: Dict[str, Any], file_name: str) -> bool:
        ...
        try:
            ...
            if not orm_instance:
                self.logger.error(f"文件[{file_name}]的batch表数据生成ORM实例失败")
                return False
            ...
            if existing_record is None:
                ...
                if inserted:
                    self.logger.info(f"文件[{file_name}]的batch表数据插入成功")
                return inserted
            else:
                # 处理更新
                ...
                if changed_fields:
                    # 执行更新
                    result = self.repo.update_batch_fields(pk_value, changed_fields)
                    
                    if result:
                        self.logger.info(f"文件[{file_name}]的batch表数据更新成功，字段: {list(changed_fields.keys())}")
                    ...
                    self.logger.info(f"文件[{file_name}]的batch表数据无变化，跳过更新")
                    return True
                    
        except Exception as e:
            self.logger.error(f"处理[{file_name}]的batch表数据失败", exc_info=True)
