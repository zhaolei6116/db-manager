...
class BatchProcessor:
    ...
    def __init__(self, db_session: Session):
        ...
        self.repo = BatchRepository(db_session)
    ...
    def process(self, data_dict: Dict[str, Any], file_name: str) -> bool:
        ...
        try:
            ...
            if pk_field not in data_dict or data_dict[pk_field] is None:
                logger.error(f"文件[{file_name}]的batch表数据缺少主键字段 '{pk_field}'")
                return False
            ...
            if self.repo.exists_by_pk(pk_value):
                logger.info(f"文件[{file_name}]的batch表数据主键 '{pk_value}' 已存在，跳过处理")
                return True
            ...
            if inserted:
                logger.info(f"文件[{file_name}]的batch表数据插入成功，主键: {pk_value}")
                return True
            else:
                logger.warning(f"文件[{file_name}]的batch表数据插入失败，可能是并发插入导致")
                return False
                
        except ValueError as e:
            logger.error(f"处理[{file_name}]的batch表数据失败：{str(e)}")
            return False
        except SQLAlchemyError as e:
            logger.error(f"数据库错误：处理[{file_name}]的batch表数据失败", exc_info=True)
            return False
        except Exception as e:
            logger.error(f"处理[{file_name}]的batch表数据失败", exc_info=True)
            return False