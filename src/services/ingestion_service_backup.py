...
class IngestionService:
    ...
    def get_new_json_files(self, json_dir: Optional[Path] = None) -> List[Path]:
        """
        获取所有新的JSON文件（查缺补漏版本）
        
        Args:
            temp_path: 临时目录路径，用于存储中间文件
    ...
    def process_all_new_files(self, json_dir: Optional[Path] = None) -> Dict[str, Any]:
        """
        循环处理所有新的JSON文件
...
def run_ingestion_process(json_dir: Optional[Path] = None) -> Dict[str, Any]:
    """
    数据录入流程入口函数，供调度器调用
